...
public class CreateMarketDialog extends ShopsDialog {
    ...
    public CreateMarketDialog(Locale locale, Key newMarketKey) {
        super(locale);
    ...
    public Dialog build() {
        ...
        DialogBase base = DialogBase.builder(translate("shops.create.title"))
                ...
                .body(List.of(
                        DialogBody.plainMessage(translate("shops.create.description"))
                ))
    ...
    private void onConfirm(Player player, DialogResponseView view) {
        ...
        if (MarketManager.INSTANCE.exists(newMarketKey)) {
            player.sendMessage(translate("shops.create.error.exists"));
            return;
        }

        MarketCreator marketCreator = new MarketCreator(newMarketKey, title, size, SlotList.of(contentSlots));
